...
<header>
    ...
    <div class="topbar-right">
        ...
        <!-- Language Selector -->
        <select id="localeSelect" onchange="changeLocale(this.value)" style="margin-right:8px;">
        ...
        <button class="auth-btn" id="authBtn" onclick="toggleAuth()"><i class="fa-brands fa-microsoft"></i> {{ __('messages.sign_in') }}</button>
    </div>
</header>
...
<div class="section" id="kb">
    ...
    <div id="kbContainer" style="display:flex;flex-direction:column;gap:12px;max-height:520px;overflow-y:auto;padding-right:4px;">
        <!-- Fetched from backend -->
    </div>
</div>
...
<div class="section" id="academic">
    ...
    <div class="card" style="margin-top:0;">
        <h3 class="section-title" id="academicYearTitle">All Modules</h3>
...
<div class="section" id="admin">
    ...
    <div style="display:flex;gap:12px;margin-bottom:24px;overflow-x:auto;padding-bottom:4px;" id="adminSubNav">
        ...
        <button class="btn btn-outline" onclick="switchAdminTab('kb')" style="border-radius:20px;padding:6px 16px;font-size:13px;" data-tab="kb"><i class="fa-solid fa-database"></i> Knowledge Base</button>
    ...
    <div id="adminView-dashboard" class="admin-view">
        ...
        <div class="grid-2">
            ...
            <div class="card">
                ...
                <p style="font-size:12px;color:var(--text-muted);margin-bottom:12px;">Add a title and category. Link and file attachment are optional.</p>
                ...
                <div class="form-group"><label>Link (Optional)</label><input type="url" id="adminKbUrl" placeholder="https://example.com/resource"></div>
                <div class="form-group">
                    <label>File Attachment (Optional)</label>
                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                        <input type="file" id="adminKbFile" accept=".pdf,.doc,.docx,.ppt,.pptx,.txt,.xls,.xlsx" style="display:none;" onchange="document.getElementById('adminKbFileName').textContent = this.files[0] ? this.files[0].name : 'No file selected'">
                        <button type="button" class="btn btn-outline" style="font-size:12px;margin:0;" onclick="document.getElementById('adminKbFile').click()"><i class="fa-solid fa-file-arrow-up"></i> Choose File</button>
                        <span id="adminKbFileName" style="font-size:12px;color:var(--text-muted);">No file selected</span>
                    </div>
                </div>
    ...
    <div id="adminView-modules" class="admin-view" style="display:none;">
        ...
        <div class="card" style="margin-top:20px;">
            ...
            <div style="max-height:360px;overflow-y:auto;overflow-x:auto;border:1px solid var(--border);border-radius:10px;">
                <table style="width:100%;">
                    <thead style="position:sticky;top:0;background:var(--surface);z-index:1;"><tr><th>Code</th><th>Name</th><th>Credits</th><th>Faculty</th><th>Action</th></tr></thead>
    ...
    <div id="adminView-kb" class="admin-view" style="display:none;">
        <div class="card">
            <h3 class="section-title">Add Knowledge Base Resource</h3>
            <p style="font-size:12px;color:var(--text-muted);margin-bottom:12px;">Title and category are required. Add a link, a file, or both.</p>
            <div class="form-group"><label>Title *</label><input type="text" id="adminKbTabTitle" placeholder="e.g. Thesis Guidelines"></div>
            <div class="grid-2">
                <div class="form-group"><label>Category *</label><input type="text" id="adminKbTabCat" placeholder="e.g. Guidelines"></div>
                <div class="form-group"><label>Source</label><input type="text" id="adminKbTabSource" placeholder="e.g. IT Faculty"></div>
            </div>
            <div class="form-group"><label>Link (Optional)</label><input type="url" id="adminKbTabUrl" placeholder="https://example.com/resource"></div>
            <div class="form-group">
                <label>File Attachment (Optional)</label>
                <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                    <input type="file" id="adminKbTabFile" accept=".pdf,.doc,.docx,.ppt,.pptx,.txt,.xls,.xlsx" style="display:none;" onchange="document.getElementById('adminKbTabFileName').textContent = this.files[0] ? this.files[0].name : 'No file selected'">
                    <button type="button" class="btn btn-outline" style="font-size:12px;margin:0;" onclick="document.getElementById('adminKbTabFile').click()"><i class="fa-solid fa-file-arrow-up"></i> Choose File</button>
                    <span id="adminKbTabFileName" style="font-size:12px;color:var(--text-muted);">No file selected</span>
                </div>
            </div>
            <button class="btn btn-primary" style="background:var(--danger);width:100%;" id="adminKbTabSubmitBtn" onclick="submitAdminKBTab()"><i class="fa-solid fa-upload"></i> Add Resource</button>
        </div>
        <div class="card" style="margin-top:20px;">
            <h3 class="section-title">Knowledge Base Resources (<span id="adminKbCount">0</span>)</h3>
            <div style="max-height:420px;overflow-y:auto;overflow-x:auto;border:1px solid var(--border);border-radius:10px;">
                <table style="width:100%;">
                    <thead style="position:sticky;top:0;background:var(--surface);z-index:1;"><tr><th>Title</th><th>Category</th><th>Source</th><th>Link / File</th><th>Action</th></tr></thead>
                    <tbody id="adminKbTable">
                        <tr><td colspan="5" style="text-align:center;color:var(--text-muted);">Loading resources...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>
...
async function apiDelete(url) {
    ...
    return apiFetch(url.replace(/\/$/, '') + '/remove', { method: 'POST' });
}
...
async function clearChatHistoryFromSettings() {
    ...
    try {
        await apiDelete('/api/v1/chat/history');
...
function initAuth() {
    ...
    const peerTab = document.getElementById('btnPeerChatTab');
    if (peerTab) peerTab.style.display = 'inline-flex';
    ...
    if(token) {
        ...
        if (role === 'admin') {
            ...
            const peerTab = document.getElementById('btnPeerChatTab');
            if (peerTab) peerTab.style.display = 'none';
            stopPeerPolling();
...
function getCurrentStudentYear() {
    const userStr = localStorage.getItem('susl_user');
    if (!userStr) return null;
    try {
        const user = JSON.parse(userStr);
        return user.year ? parseInt(user.year, 10) : null;
    } catch (e) {
        return null;
    }
}
...
function updateAcademicYearLabel() {
    const titleEl = document.getElementById('academicYearTitle');
    if (!titleEl) return;

    const year = getCurrentStudentYear();
    titleEl.textContent = year ? `All Modules — Year ${year}` : 'All Modules';
}

function getVisibleModules() {
    const year = getCurrentStudentYear();
    if (!year) return MODULES_DB;
    return MODULES_DB.filter(m => !m.year || parseInt(m.year, 10) === year);
}
...
function doSearch() {
    ...
    const results = getVisibleModules().filter(m =>
...
function renderAllModules() {
    ...
    if (!modules || !modules.length) {
        container.innerHTML = '<div class="card"><p style="color:var(--text-muted);">No modules available for your year yet.</p></div>';
        return;
    }
    ...
    try {
        ...
        if(data && data.timetable) {
            ...
            for(const [fullDay, classes] of Object.entries(data.timetable)) {
                ...
                classes.forEach(c => {
                    const slotIndex = TT_SLOTS.findIndex(s => normalizeTime(s.split('–')[0]) === normalizeTime(c.start_time));
                    if(slotIndex !== -1) {
    ...
    reader.onload = async function(e) {
        ...
        try {
            ...
            for (const ev of parsedEvents) {
                const slotIndex = TT_SLOTS.findIndex(s => normalizeTime(s.split('–')[0]) === normalizeTime(ev.start_time));
...
async function adminDeletePost(postId) {
    ...
    try {
        await apiDelete('/api/v1/admin/communities/' + postId);
...
async function submitAdminKBTab() {
    const title = document.getElementById('adminKbTabTitle').value.trim();
    const cat = document.getElementById('adminKbTabCat').value.trim();
    const source = document.getElementById('adminKbTabSource').value.trim();
    let url = document.getElementById('adminKbTabUrl').value.trim();
    const fileInput = document.getElementById('adminKbTabFile');

    if (!title || !cat) {
        addNotif('⚠️ Title and Category are required!');
        return;
    }
    ...
    const submitBtn = document.getElementById('adminKbTabSubmitBtn');
    const origHtml = submitBtn.innerHTML;
    submitBtn.disabled = true;
    submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Uploading...';
    ...
    try {
        ...
        addNotif('Resource added to Knowledge Base!');
        document.getElementById('adminKbTabTitle').value = '';
        document.getElementById('adminKbTabCat').value = '';
        document.getElementById('adminKbTabSource').value = '';
        document.getElementById('adminKbTabUrl').value = '';
        if (fileInput) fileInput.value = '';
        document.getElementById('adminKbTabFileName').textContent = 'No file selected';
...
async function loadAdminKB() {
    try {
        const items = await apiFetch('/api/v1/knowledge-bases');
        allKB = Array.isArray(items) ? items : [];

        const countEl = document.getElementById('adminKbCount');
        if (countEl) countEl.textContent = allKB.length;

        const tbody = document.getElementById('adminKbTable');
        if (!tbody) return;
        ...
        tbody.innerHTML = allKB.map(k => `
            <tr>
                <td style="font-weight:600;">${escHtml(k.title)}</td>
                <td>${escHtml(k.category || '-')}</td>
                <td>${escHtml(k.source || '-')}</td>
                <td style="font-size:12px;">
                    ${k.url ? `<a href="${k.url}" target="_blank" style="color:var(--primary);">Link</a>` : ''}
                    ${k.url && k.file_path ? ' · ' : ''}
                    ${k.file_path ? `<a href="${k.file_path}" target="_blank" style="color:var(--danger);">File</a>` : (!k.url ? '-' : '')}
                </td>
                <td><button class="btn btn-outline" style="color:var(--danger);border-color:var(--danger);padding:4px 8px;font-size:12px;margin:0;" onclick="deleteAdminKB(${k.id})"><i class="fa-solid fa-trash"></i></button></td>
            </tr>
        `).join('');
    } catch (e) {
...
async function deleteAdminKB(id) {
    if (!confirm('Delete this knowledge base resource?')) return;
    try {
        await apiDelete('/api/v1/knowledge-bases/' + id);
        addNotif('Resource deleted.');
        fetchKB();
        loadAdminKB();
    } catch (e) {
        addNotif('⚠️ Error deleting resource: ' + e.message);
    }
}
...
async function deleteAdminModule(id) {
    ...
    try {
        await apiDelete('/api/v1/academic-modules/' + id);
...
function cachePeer(peer) {
    if (peer && peer.student_id) {
        peerDirectory[peer.student_id] = peer;
    }
}

function selectPeerByStudentId(studentId) {
    const peer = peerDirectory[studentId];
    if (!peer) {
        addNotif('⚠️ Could not open chat for this student.');
        return;
    }
    selectPeer(peer);
}
...
function switchChatTab(tab) {
    if (tab === 'peer') {
        ...
        if (!isLoggedIn || role !== 'student') {
            addNotif('⚠️ Please sign in as a student to use Student Chat.');
            toggleAuth();
            return;
        }
    }
...
async function loadRecentPeerChats() {
    ...
    try {
        ...
        container.innerHTML = res.map(conv => {
            ...
            return `
                <div onclick="selectPeerByStudentId('${escHtml(peer.student_id)}')" style="display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:10px;cursor:pointer;transition:background 0.2s;background:${isActive ? 'rgba(128,0,0,0.05)' : 'transparent'};border-left:${isActive ? '3px solid #800000' : 'none'};" class="peer-item-hover">
                    ${avatarHtml}
    ...
    peerSearchTimeout = setTimeout(async () => {
        try {
            ...
            dropdown.innerHTML = res.map(st => {
                ...
                return `
                    <div onclick="selectPeerByStudentId('${escHtml(st.student_id)}')" style="display:flex;align-items:center;gap:10px;padding:8px 10px;cursor:pointer;" class="peer-item-hover">
                        ${avatarHtml}
    ...
    try {
        ...
        tbody.innerHTML = data.map(rep => {
            ...
            if (rep.reported_post_id) {
                ...
                reportedEntity = `<div><strong>Student Report</strong><div style="font-size:10px;color:var(--text-muted);">${name} (${sId})</div></div>`;
            }
